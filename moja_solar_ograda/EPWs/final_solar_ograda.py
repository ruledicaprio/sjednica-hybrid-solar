...
# =========================== 3D VIZUALIZACIJA SA CRVENIM PUNIM PANELIMA ===========================
print("\n🖼️ Generišem 3D vizualizaciju i bočne prikaze (paneli crveni, puna boja)...")

def draw_3d_model(ax):
    """Iscrtavanje 3D modela sa crvenim punim panelima"""
    panel_width = PANEL_X
    ...
    # Podaci o stranama (sada svi paneli crveni, bez obzira na stranu)
    side_data = {
    ...
    panel_color = 'red'       # svi paneli crveni
    edge_color = 'black'      # ivice crne
    ...
    for side_name, data in side_data.items():
        ...
        if side_name == "Southeast":
            ...
            for i in range(2):
                ...
                # Dodaj puni panel (Poly3DCollection)
...
draw_3d_model(ax_main)
ax_main.set_title(f'SJEDNICA, BILECA – 3D model solarne ograde\nPaneli crveni, nagib {TILT_DEG:.1f}° prema van, platforma na 4,2 m', fontsize=12)
...
ax_east.set_title('Pogled sa istoka (East view) – crveni paneli')
plt.tight_layout()
...
ax_north.set_title('Pogled sa sjevera (North view) – crveni paneli')
plt.tight_layout()
north_image = os.path.join(BASE_PATH, 'SJEDNICA_BILECA_pogled_sjever.png')
plt.savefig(north_image, dpi=150)
plt.close(fig_north)

print("✅ 3D model i bočni prikazi sačuvani (svi paneli crveni, puna boja).")
...
with PdfPages(pdf_path) as pdf:
    ...
    if os.path.exists(output_image):
        fig = plt.figure(figsize=(11.69, 8.27))
        plt.imshow(plt.imread(output_image)); plt.axis('off'); plt.title('3D model solarne ograde (crveni paneli)')
        pdf.savefig(fig); plt.close(fig)
    
    # Bočni prikazi
    fig_side = plt.figure(figsize=(11.69, 8.27))
    ax_e = fig_side.add_subplot(1,2,1)
    if os.path.exists(east_image):
        ax_e.imshow(plt.imread(east_image)); ax_e.axis('off'); ax_e.set_title('Pogled sa istoka')
    ax_n = fig_side.add_subplot(1,2,2)
    if os.path.exists(north_image):
        ax_n.imshow(plt.imread(north_image)); ax_n.axis('off'); ax_n.set_title('Pogled sa sjevera')
    pdf.savefig(fig_side); plt.close(fig_side)
...
print(f"   - 3D model (crveni paneli): {output_image}")
print(f"   - Bočni prikazi: {east_image}, {north_image}")
print(f"   - PDF izvještaj: {pdf_path}")
print("\n✅ Simulacija, vizualizacija i izvještaj završeni. Svi paneli su crveni i popunjeni (fill).")