...
parameterised so both stand layouts (3 stands of 2x2 modules, "3x4", or 2
stands of 2x3, "2x6") and both tilts (45° / 60°) can be compared per site.
...
LAYOUTS = {
    "3x4": {"stands": 3, "cols": 2, "rows": 2, "frame_kg": 110.0,
            "_frame": "design.json support.weight_kg (estimate, fabricator confirms)"},
    "2x6": {"stands": 2, "cols": 3, "rows": 2, "frame_kg": 159.0,
...
def stand(layout, tilt, qp, module, bottom_edge=0.5, strip_spacing=1.6, c_f=1.5):
    ...
    return {
        ...
        "strip_m3_adopted": max(strip_req, REV8_STRIP_M3),
        "strips": 2 * lay["stands"],
        "concrete_m3_total": 2 * lay["stands"] * max(strip_req, REV8_STRIP_M3),
